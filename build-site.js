...
function serviceCards() {
  ...
  return `<div class="service-grid">${services.map((s, i) => {
    const isSmallerWorks = i === 6;
    const title = isSmallerWorks ? "Ook voor kleinere dakwerken" : s[0];
    const text = isSmallerWorks ? "Heeft u een kleiner dakwerk, herstelling of afwerking rond woning of bijgebouw? Ook dat kan besproken worden met Werner." : copy[i][1];
    const cta = isSmallerWorks ? "Meer over kleinere dakwerken" : "Meer info";
    return `<a class="service-card${isSmallerWorks ? " service-card-highlight" : ""}" href="${s[1]}">${img(s[2], s[3])}<div><span>${copy[i][0]}</span><h3>${title}</h3><p>${text}</p><b>${cta}${isSmallerWorks ? "" : " →"}</b></div></a>`;
  }).join("")}</div>`;
}
...
function homeBody(page) {
  ...
  <section class="soft home-area-section"><div class="container area"><span class="kicker">Werkgebied</span><h2>Dakwerken regio Limburg</h2><p>Vanuit Wellen is Werner actief in de ruime regio Limburg. Denk aan Wellen, Hasselt, Sint-Truiden, Tongeren, Borgloon, Bilzen, Diepenbeek en omliggende gemeenten. Afhankelijk van het project kunnen ook werken net buiten Limburg besproken worden.</p></div></section>
...
function aboutBody(page) {
  const trustItems = [
    ["01", "Duidelijk aanspreekpunt", "U bespreekt de situatie rechtstreeks met Werner."],
    ["02", "Praktisch bekeken", "Er wordt gekeken wat nodig is en welke aanpak logisch is."],
    ["03", "Heldere afspraken", "U krijgt duidelijke uitleg voor de werken starten."],
  ];
  return `<main class="about-premium-page">${breadcrumbs(page)}
  <section class="soft about-page-top"><div class="container"><div class="split about-premium-grid"><figure class="photo about-premium-photo">${img(page.image, page.alt)}</figure><div class="about-premium-copy"><span class="kicker">Over Vandormael Werner</span><h1>Rechtstreeks contact met een lokale dakwerker uit Limburg.</h1><p>Bij Vandormael Werner spreekt u niet met een grote firma of tussenpersoon. U neemt rechtstreeks contact op met Werner, die de situatie mee bekijkt en duidelijk uitlegt wat mogelijk is.</p><p>De focus ligt op algemene dakwerken, dakisolatie, hellende daken, platte daken, dakherstellingen en afwerking in zink, koper en lood. Kleinere dakwerken of praktische klussen kunnen mee besproken worden.</p><ul class="checks"><li>Lokale dakwerker</li><li>Actief in Limburg en omliggende regio</li><li>Voor hellende en platte daken, dakisolatie, dakherstellingen en kleinere klussen</li><li>Duidelijke communicatie en praktische afspraken</li><li>Bereikbaar via telefoon, e-mail, WhatsApp of contactformulier</li></ul><div class="about-card-action"><a class="btn btn-red" href="/contact/">Contact opnemen met Werner</a></div></div></div></div></section>
  <section class="about-trust-section"><div class="container"><div class="premium-section-heading"><span class="kicker">Rechtstreeks contact</span><h2>Waarom rechtstreeks contact belangrijk is</h2></div><div class="about-trust-grid">${trustItems.map(([number, title, text]) => `<article><span>${number}</span><h3>${title}</h3><p>${text}</p></article>`).join("")}</div></div></section>
  <section class="about-bottom-cta-section"><div class="container premium-bottom-cta about-bottom-cta"><div><span class="kicker">Contact</span><h2>Dakwerk bespreken met Werner?</h2><p>Heeft u een vraag over uw dak, dakisolatie, dakherstelling of kleinere dakwerken? Bel Werner, stuur een WhatsApp-bericht of stuur een e-mail.</p></div><div class="cta-actions"><a class="btn btn-white" href="tel:${phone}">Bel Werner</a><a class="btn btn-outline-light" href="${whatsapp}" target="_blank" rel="noopener">WhatsApp Werner</a><a class="btn btn-outline-light premium-email-cta" href="mailto:${email}"><span>Stuur een e-mail</span><small>${email}</small></a></div></div></section></main>`;
}
...
function contactBody(page) {
  ...
  return `<main class="premium-contact-page">${breadcrumbs(page)}
  <section class="soft premium-contact-top"><div class="container split premium-contact-intro"><div class="premium-contact-copy"><span class="kicker">Contact</span><h1>Contact opnemen met Werner</h1><p>Heeft u een vraag over uw dak, dakisolatie, dakherstelling of kleinere dakwerken? Bel Werner, stuur een WhatsApp-bericht of stuur een e-mail.</p><ul class="checks"><li>Rechtstreeks contact met Werner</li><li>Actief in de ruime regio Limburg</li><li>Duidelijke uitleg voor de werken starten</li><li>Bereikbaar via telefoon, WhatsApp of e-mail</li></ul><div class="contact-options">${options.map(([title, text, href], index) => `<a class="contact-option" href="${href}" ${href.startsWith("https://") ? 'target="_blank" rel="noopener"' : ""}><b>0${index + 1}</b><span>${title}</span><strong>${text}</strong></a>`).join("")}</div><p class="contact-helper">U kan via WhatsApp of e-mail ook enkele foto's meesturen, zodat Werner de situatie eerst kan bekijken.</p></div><figure class="photo premium-contact-photo">${img("werner-vandormael-dakwerker-limburg.jpg", "Werner Vandormael aan het werk als dakwerker in Limburg")}</figure></div></section>
  <section class="premium-contact-form-section"><div class="container"><div class="premium-section-heading"><span class="kicker">Uw vraag</span><h2>Vertel kort waarmee Werner kan helpen</h2><p>Vul de gegevens in en kies daarna e-mail of WhatsApp. U kan in de geopende app eventueel foto's toevoegen.</p></div><div class="contact-page"><form class="contact-card contact-form" action="mailto:${email}" method="post" enctype="text/plain" data-email="${email}" data-whatsapp="https://wa.me/32495548415"><h2>Vraag een vrijblijvende inschatting</h2><p>Dit formulier opent uw e-mailprogramma of WhatsApp met de ingevulde gegevens. U kan daarna eventueel foto's toevoegen.</p><div class="contact-form-fields">${[["Naam","text"],["Telefoon","tel"],["E-mail","email"],["Gemeente","text"],["Type dakwerk","text"]].map(x=>`<label>${x[0]}<input name="${x[0]}" type="${x[1]}" ${x[0]==="Naam"||x[0]==="E-mail"?"required":""}></label>`).join("")}<label class="contact-message">Bericht<textarea name="Bericht" rows="6" required></textarea></label></div><div class="actions"><button class="btn btn-red" type="submit">Verstuur via e-mail</button><a class="btn btn-outline contact-whatsapp-submit" href="${whatsapp}" target="_blank" rel="noopener">Verstuur via WhatsApp</a></div></form><aside class="contact-address"><span class="kicker">Contactgegevens</span><h2>Vandormael Werner</h2><address><span>Plattestraat 33</span><span>3830 Wellen</span><span>België</span></address><div class="contact-address-links"><a href="tel:${phone}"><small>Telefoon</small><strong>${phoneDisplay}</strong></a><a href="mailto:${email}"><small>E-mail</small><strong>${email}</strong></a></div></aside></div></div></section></main>`;
}
