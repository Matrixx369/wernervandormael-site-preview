...
function footer() {
  return `<footer><div class="footer-grid container"><div><a class="brand brand-footer" href="/"><img src="/assets/images/vandormael-werner-logo.svg" alt="Vandormael Werner Dakwerken en Klussen" width="390" height="155"></a><p>Vandormael Werner voert dakwerken uit in de ruime regio Limburg, met Wellen als uitvalsbasis.</p></div><div><h2>Diensten</h2>${services.map(s => `<a href="${s[1]}">${s[0]} Limburg</a>`).join("")}</div><div class="footer-contact"><h2>Contact</h2><address><span>Vandormael Werner</span><span>Plattestraat 33</span><span>3830 Wellen</span><span>België</span></address><span class="footer-contact-divider" aria-hidden="true">·</span><div class="footer-contact-links"><a href="tel:${phone}">${phoneDisplay}</a><a href="mailto:${email}">${email}</a><a href="${whatsapp}" target="_blank" rel="noopener">WhatsApp Werner</a></div></div><div><h2>Informatie</h2><a href="/over-vandormael-werner/">Over Vandormael Werner</a><a href="/contact/">Contact</a><a href="/privacybeleid/">Privacybeleid</a><a href="/algemene-voorwaarden/">Algemene voorwaarden</a></div></div><div class="copy container">© ${new Date().getFullYear()} Vandormael Werner · Algemene dakwerken</div></footer><a class="mobile-call btn btn-red" href="tel:${phone}">Bel Werner</a>`;
}
...
function contactCta(servicePage = false) {
  ...
  const copy = servicePage
    ? "Heeft u een vraag over deze werken? Bel Werner, stuur een WhatsApp-bericht of stuur enkele foto's via e-mail."
    : "Heeft u een vraag? Bel Werner, stuur een WhatsApp-bericht of stuur een e-mail.";
  return `<section class="compact-cta-section"><div class="container compact-cta"><div><span class="kicker">Contact</span><h2>${heading}</h2><p>${copy}</p></div><div class="actions"><a class="btn btn-red" href="tel:${phone}">Bel Werner</a><a class="btn btn-outline" href="${whatsapp}" target="_blank" rel="noopener">WhatsApp Werner</a><a class="btn btn-outline" href="mailto:${email}">Stuur een e-mail</a></div></div></section>`;
}

function homeContact() {
  return `<section class="dark home-contact" id="contact"><div class="container home-contact-inner"><span class="kicker">Contact</span><h2>Contact opnemen met Vandormael Werner</h2><p>Wilt u een dakwerk bespreken? Bel Werner of stuur een WhatsApp-bericht. U kan ook enkele foto's via e-mail of WhatsApp meesturen, zodat de situatie eerst bekeken kan worden.</p><div class="actions"><a class="btn btn-red" href="tel:${phone}">Bel Werner</a><a class="btn btn-outline-light" href="${whatsapp}" target="_blank" rel="noopener">WhatsApp Werner</a><a class="btn btn-outline-light" href="mailto:${email}">Stuur een e-mail</a></div></div></section>`;
}
...
function homeBody(page) {
  ...
  <section class="trust"><div class="container trust-grid"><article><strong>Lokale dakwerker</strong><p>Rechtstreeks contact met Werner zelf.</p></article><article><strong>Dakherstelling &amp; dakisolatie</strong><p>Voor kleinere werken én grotere dakprojecten.</p></article><article><strong>Duidelijke communicatie</strong><p>Heldere afspraken voor dakwerken, isolatie en herstellingen.</p></article></div></section>
...
function termsBody(page) {
  ...
  return `<main>${breadcrumbs(page)}<section class="page-hero"><div class="container"><span class="eyebrow">Praktische afspraken</span><h1>${page.h1}</h1><p>Deze algemene voorwaarden zijn van toepassing op offertes, afspraken en uitgevoerde werken door Vandormael Werner, tenzij schriftelijk anders overeengekomen.</p></div></section><section><article class="container prose"><p><strong>Deze pagina bevat praktische standaardvoorwaarden en vormt geen formeel juridisch advies.</strong></p>${sections.map(([h, p]) => `<h2>${h}</h2><p>${p}</p>`).join("")}<h2>Contact</h2><address class="legal-contact"><span>Vandormael Werner</span><span>Plattestraat 33</span><span>3830 Wellen</span><span>België</span><span aria-hidden="true">·</span><a href="tel:${phone}">${phoneDisplay}</a><a href="mailto:${email}">${email}</a></address><p>Laatst bijgewerkt: 8 juni 2026.</p></article></section></main>`;
}
