...
const whatsapp = "https://wa.me/32495548415";
...
function header() {
  return `<header><nav class="nav container" aria-label="Hoofdnavigatie"><a class="brand" href="/" aria-label="Vandormael Werner home"><img src="/assets/images/vandormael-werner-logo.svg" alt="Vandormael Werner Dakwerken en Klussen" width="390" height="155"></a><button class="menu-toggle" aria-expanded="false" aria-controls="main-menu">Menu</button><div class="menu" id="main-menu"><a href="/dakwerken/limburg/">Dakwerken</a><details><summary>Diensten</summary><div class="dropdown">${services.map(s => `<a href="${s[1]}">${s[0]}</a>`).join("")}</div></details><a href="/over-vandormael-werner/">Over</a><a href="/contact/">Contact</a></div><a class="btn btn-red nav-call" href="tel:${phone}">Bel Werner</a></nav></header>`;
}

function footer() {
  return `<footer><div class="footer-grid container"><div><a class="brand brand-footer" href="/"><img src="/assets/images/vandormael-werner-logo.svg" alt="Vandormael Werner Dakwerken en Klussen" width="390" height="155"></a><p>Vandormael Werner voert dakwerken uit in de ruime regio Limburg, met Wellen als uitvalsbasis.</p></div><div><h2>Diensten</h2>${services.map(s => `<a href="${s[1]}">${s[0]} Limburg</a>`).join("")}</div><div><h2>Contact</h2><address>Vandormael Werner<br>Plattestraat 33<br>3830 Wellen<br>België</address><a href="tel:${phone}">Tel: ${phoneDisplay}</a><a href="mailto:${email}">E-mail: ${email}</a><a href="${whatsapp}" target="_blank" rel="noopener">WhatsApp: ${phoneDisplay}</a></div><div><h2>Informatie</h2><a href="/over-vandormael-werner/">Over Vandormael Werner</a><a href="/contact/">Contact</a><a href="/privacybeleid/">Privacybeleid</a><a href="/algemene-voorwaarden/">Algemene voorwaarden</a></div></div><div class="copy container">© ${new Date().getFullYear()} Vandormael Werner · Algemene dakwerken</div></footer><a class="mobile-call btn btn-red" href="tel:${phone}">Bel Werner</a>`;
}
...
function contactBody(page) {
  return `<main>${breadcrumbs(page)}<section class="page-hero"><div class="container"><span class="eyebrow">Dakwerken Limburg</span><h1>${page.h1}</h1><p>Neem contact op om uw dakwerk te bespreken of stuur enkele foto's van de situatie door.</p></div></section><section><div class="container contact-grid"><div class="contact-card"><h2>Vandormael Werner</h2><div class="contact-details"><div><small>Adres</small><address>Plattestraat 33<br>3830 Wellen<br>België</address></div><a href="tel:${phone}"><small>Telefoon</small><strong>${phoneDisplay}</strong></a><a href="${whatsapp}" target="_blank" rel="noopener"><small>WhatsApp</small><strong>${phoneDisplay}</strong></a><a href="mailto:${email}"><small>E-mail</small><strong>${email}</strong></a></div><div class="actions"><a class="btn btn-red" href="tel:${phone}">Bel Werner</a><a class="btn btn-outline" href="${whatsapp}" target="_blank" rel="noopener">Stuur via WhatsApp</a><a class="btn btn-outline" href="mailto:${email}">Stuur een e-mail</a></div></div><form class="contact-card" action="mailto:${email}" method="post" enctype="text/plain"><h2>Vraag een vrijblijvende inschatting</h2><p>Dit formulier opent uw e-mailprogramma. Voeg daar eventueel foto's toe.</p>${[["Naam","text"],["Telefoon","tel"],["E-mail","email"],["Gemeente","text"],["Type dakwerk","text"]].map(x=>`<label>${x[0]}<input name="${x[0]}" type="${x[1]}" ${x[0]==="Naam"||x[0]==="E-mail"?"required":""}></label>`).join("")}<label>Bericht<textarea name="Bericht" rows="6" required></textarea></label><button class="btn btn-red" type="submit">Stuur uw aanvraag</button></form></div></section></main>`;
}
